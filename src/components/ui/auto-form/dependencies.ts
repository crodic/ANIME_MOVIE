...
export default function useDependencies(fieldName: string) {
  ...
  function getSourceValue(dep: Dependency<any>) {
    ...
    const [sourceLast, ...sourceInitial] = (source.split('.') as any).toReversed()
    const [_targetLast, ...targetInitial] = (
      (dep.targetField as string).split('.') as any
    ).toReversed()

    if (index >= 0 && sourceInitial.join(',') === targetInitial.join(',')) {
      const [_currentLast, ...currentInitial] = (fieldName.split('.') as any).toReversed()
